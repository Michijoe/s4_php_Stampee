...
function afficherModaleJonction() {
  ...
  fetch('membre?entite=' + entite + '&action=lf&film_id=' + film_id)
    .then(reponse => {
  ...
  eDeposable.addEventListener("drop", (evt) => {
    ...
    if (contexte.statut === 'jonctionDisponibles' && eDeposable.id === 'jonctionPris'
      ...
      if (priorite) {
        ...
        uri = 'membre?entite=' + entite + '&action=mf&' + entite + '_id=' + itemsPrisBis.toString() + '&film_id=' + film_id;
      } else {
        uri = 'membre?entite=' + entite + '&action=af&' + entite + '_id=' + contexte.idj + '&film_id=' + film_id;
      }
      fetch(uri)
        .then(reponse => {
          if (!reponse.ok) throw { message: "Problème technique sur le serveur" };
    ...
    if (contexte.statut === 'jonctionPris' && eDeposable.id === 'jonctionDisponibles') {
      ...
      if (priorite) {
        ...
        uri = 'membre?entite=' + entite + '&action=sf&' + entite + '_id=' + contexte.idj + '&film_id=' + film_id;
      }
      fetch(uri)
        .then(reponse => {
          if (!reponse.ok) throw { message: "Problème technique sur le serveur" };
